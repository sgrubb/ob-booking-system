...
import { registerContactHandlers } from "./handlers/contact-handlers";
...
async function openMainWindow(): Promise<void> {
  ...
  registerSeasonHandlers(ipcMain, prisma);
  registerMemberHandlers(ipcMain, prisma);
  registerContactHandlers(ipcMain, prisma);
  registerBookingHandlers(ipcMain, prisma);
  registerSettingsHandlers(ipcMain, dialog, prisma);
  registerExportHandlers(ipcMain, dialog, prisma);
  log.info("Domain handlers registered");
...
async function bootstrap() {
  log.info(`RFP OB Bookings ${app.getVersion()} starting (${IS_DEV ? "dev" : "prod"})`);
  ...
  if (schemaVersion < CURRENT_SCHEMA_VERSION) {
    ...
    log.info(
      `Migration window created (DB version ${schemaVersion}, required ${CURRENT_SCHEMA_VERSION})`,
    );
...
bootstrap().catch((err) => {
  ...
  dialog.showErrorBox(
    "RFP OB Bookings failed to start",
